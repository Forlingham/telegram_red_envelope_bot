...
```
发送者输入 @username → 检查用户是否存在 →
  ├─ 用户已存在 → 检查是否绑定钱包
  │   ├─ 已绑定钱包 → 直接转账到用户地址
  │   └─ 未绑定钱包 → 金额转入统筹账户，创建待转账记录
  └─ 用户不存在 → 创建临时用户（telegramId = temp_username_timestamp）
      → 检查是否绑定钱包（同上）
```
...
```
发送者输入金额和份数 → 资金转入统筹账户（包含手续费储备）
→ 用户点击抢红包 →
  ├─ 已绑定钱包 → 直接到账
  └─ 未绑定钱包 → 创建待转账记录，资金保留在统筹账户
```

#### 3. 活跃红包/抽奖红包流程

```
选择红包类型 → 选择抽奖范围（全群/前50/前100）→ 输入中奖人数
→ 输入金额 → 选择分配方式（均分/随机/按活跃度排序）
→ 系统查询最近30分钟活跃用户 →
  ├─ 有地址用户 → 直接转账
  └─ 无地址用户 → 金额转入统筹账户，创建待转账记录
```
...
1. 创建临时用户记录：
   - `telegramId`: `temp_${username}_${timestamp}`
   - `username`: 用户名
   - `firstName`: "Unknown"
   - `isWatchOnly`: true
...
2. 用户首次与机器人互动时：
   ...
   - 将 isWatchOnly 设为 false

#### 5. 用户未绑定钱包处理

当红包目标用户没有绑定钱包地址时：

1. **金额处理**：该用户应得金额转入统筹账户
2. **记录创建**：
   - `red_packet_claims`: 状态 PENDING，记录应得金额
   - `pooling_transfers`: 状态 PENDING，等待用户绑定后自动转账
3. **用户绑定钱包后**：定时任务自动处理转账
